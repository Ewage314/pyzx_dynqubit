...
from ..routing.phase_poly import make_random_phase_poly, make_random_phase_poly_from_gadgets

description = "Generates random phase polynomial circuits and stores them as QASM files."
...
def main(args):
    parser.add_argument("-p", "--n_phase_gadgets", nargs="+", default=1, type=int, help="The number of phase gadgets to be generated in the circuit.")
...
def main_old(args):
    parser.add_argument("-d", "--n_cnots", nargs='+', default=1, type=int, help="The number of CNOTs per layer in the generated circuit.")
    parser.add_argument("-p", "--n_phase_layers", nargs='+', default=1, type=int, help="The number of phase layers in the generated circuit.")
    args = parser.parse_args(args)
    ...
    for q in n_qubits:
        for p in n_phase_layers:
            for n in n_cnots:
                dest_folder = os.path.join(folder, str(q) + "qubits", str(p)+"layers"+ str(n)+"cnots")
                os.makedirs(dest_folder, exist_ok=True)
                for i in range(n_maps):
                    filename = "Original" + str(i) + ".qasm"
                    dest_file = os.path.join(dest_folder, filename)
